...
public class DispenseProductState implements State {
	DispenseProductState(VendingMachine vm, int code) throws Exception {
		proudctDispense(vm, code);
	}
	...
	public void proudctDispense(VendingMachine vm, int code) throws Exception{
		...
		List<ItemShelf> items = vm.getInventory().getItemList().stream().filter((ItemShelf it) -> it.getCode() == code && it.isItemAvailable() == false).collect(Collectors.toList());
